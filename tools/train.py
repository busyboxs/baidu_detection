...
def main():
    ...
    parameters.init_from_tar(gzip.open('models/Paddle_Vgg16.tar.gz', 'r'))
    ...
    train_reader = paddle.batch(
        paddle.reader.shuffle(
            ...
            reader.train_reader('data/split/train.txt'),
            buf_size=1000),
        batch_size=BATCH_SIZE)
    test_reader = paddle.batch(
        # flowers.valid(),
        # To use other data, replace the above line with:
        reader.test_reader('data/split/val.txt'),
        batch_size=BATCH_SIZE)
    ...
    def event_handler(event):
        ...
        if isinstance(event, paddle.event.EndPass):
            with gzip.open('/media/yangshun/0008EB70000B0B9F/mlcouplet/baidu_classification/params_pass_%d.tar.gz' % event.pass_id, 'w') as f:
